...
export class ApiService {
  ...
  get(endpoint): Promise<any> {
    // let HOST = this.getApiHost(endpoint)
  ...
  getApiHost(endpoint: string): string {
    let routes: string[] = endpoint.split("/")
    let API_HOST_TYPE = ""
    for(var i = 0; i < routes.length; i++) {
        if (routes[i] == USERS) {
          API_HOST_TYPE = USERS
        } else if (routes[i] == FEED) {
          API_HOST_TYPE = FEED
        }
    }

    return API_HOST_TYPE === USERS ? environment.apiHost : environment.apiHost
  }
}
